...
- 服务端会写入 7 条示例线索模板（覆盖企业、学校、组织等场景）及评论、通知、访问统计
- 每个浏览器首次访问时会复制一份独立的示例副本，示例记录、未读状态和补充回复互不影响
- 后台可同时看到示例模板与访客提交的真实线索，处理流程完全联动

演示密码仅用于功能展示。正式部署前必须设置 `DEMO_MODE=0`，同时配置真实的 `ADMIN_PASSWORD` 和至少 32 位的 `SESSION_SECRET`；关闭演示模式后 `demo12345678` 立即失效，库中的示例数据只是从界面隐藏，真实提交不会丢失。

### 数据隔离与恢复码

- 每台浏览器由服务端签发的 HttpOnly Cookie、浏览器安装密钥共同绑定，只能读写自己身份名下的提交、通知和沟通记录
- 其他人即使知道设备 ID 也无法查看记录；后台管理员登录后才能看到全部真实线索
- “我的提交”页面会展示本机恢复码，换浏览器或清除浏览器数据后输入恢复码即可找回原来的记录
- `DEMO_MODE=0` 时不再生成新的示例副本，通知、已读状态与沟通逻辑恢复为正式逻辑
